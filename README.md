# ProTimer.club

**ProTimer.club** is a modern solution for athletes, providing precise time measurement, result recording, and analysis to enhance training efficiency. Our devices are suitable for various sports and competitions, offering convenience, accuracy, and cloud integration to users.

<img src="https://github.com/renat2985/protimer.club/blob/main/doc/protimer2.png" width="300px" alt="ProTimer Image 1"> <img src="https://github.com/renat2985/protimer.club/blob/main/doc/protimer3.png" width="300px" alt="ProTimer Image 2">

## Key Features

- **Precise time measurement:** Accuracy up to 0.003 seconds.
- **Ease of use:** Simple system setup accessible to everyone.
- **Cloud integration:** Export data to ProTimer.club for storing and analyzing training history.
- **Social network:** Share achievements and follow the results of friends.
- **Versatility:** Support for various modes and features for team and individual training.

## System Components

### Master Point
The main module of the system that ensures control and accurate time measurement.  
**Features:**
- Wi-Fi hotspot for system connectivity.
- Chronometer with a built-in display.
- Supports BMX Random Gate Start, result history, and cloud export.
- Connection to electromagnetic barriers.

### Points (Modules)

Additional modules to expand the system's capabilities:  
- **Basic:** Essential features for individual use.  
- **PRO:** Equipped with a display, channel selection, and additional modes.

#### Start, Interim, and Finish Points

Modules for starting, intermediate splits, and stopping measurements:  
- Support up to 15 intermediate points.  
- Operate up to 250 meters from the Master Point.  

#### PRO Point

An advanced module with a display for professional use.  
**Modes:**
- Start
- Intermediate split
- Finish
- BMX Random Gate Start

## Contact

You can assemble the device yourself or request a pre-assembled one. To order a ready-made device, contact me via [Telegram](https://t.me/ESPiotDevice), [Skype](https://skype:renat2985?chat), or [Discord](https://discord.com/invite/zaGaDuGe). 

Ready-made equipment kits with customization options and additional modules are also available on our website.


## Connection Diagram

NodeMCU ESP8266: This diagram works for both the Master and Point (PRO) devices. Select the appropriate link below to install the software.

<img src="https://github.com/renat2985/bitball.club/blob/main/doc/schematic.png" width="400px" alt="Connection Diagram">

## STL (STL Folder)

Holders for reflectors that can be attached to tripods. It's recommended to use round reflectors as they perform better than square ones.  

<img src="https://github.com/renat2985/protimer.club/blob/main/stl/mirror4.png" height="300px" alt="Reflector Holder 1"> <img src="https://github.com/renat2985/protimer.club/blob/main/stl/mirror5.png" height="300px" alt="Reflector Holder 2">


### From AliExpress.com

<sup>https://www.aliexpress.com/item/1005004123351251.html </sup>

<sup>https://www.aliexpress.com/item/1005004177317958.html </sup>

<sup>https://www.aliexpress.com/item/32990489226.html </sup>

<sup>https://www.aliexpress.com/item/1005003584568763.html </sup>

<sup>https://www.aliexpress.com/item/1005004118377699.html </sup> 

## Web Installer (Recommended)

### Follow the instructions on the web installer page.

- **Master:** [https://renat2985.github.io/protimer.club/](https://renat2985.github.io/protimer.club/)  
- **Point (PRO):** [https://renat2985.github.io/protimer.club/point.html](https://renat2985.github.io/protimer.club/point.html)  

## :battery: Donation

If you like this project, you can support us by buying a cup of coffee! ☕  

<img src="https://github.com/renat2985/renat2985/raw/main/donate/donate.png" width="100%" alt="Donate Image">

- **PayPal:** [https://www.paypal.me/RKevrels](https://www.paypal.me/RKevrels/5)

This text has been carefully formatted in Markdown for clarity and easy reading on GitHub. It includes headings, bullet points, images, and links while maintaining a professional tone.